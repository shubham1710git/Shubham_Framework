...
import com.xm.qa.interfaces.Action;
    ...
    private static void waitUntilElementActionCompletes(WebElement element, Action action) {
        new FluentWait<>(element).withTimeout(UtilPage.Implicit_Wait, TimeUnit.SECONDS)
                .pollingEvery(UtilPage.Implicit_Wait, TimeUnit.MILLISECONDS).ignoring(WebDriverException.class)
    ...
    public static String getTextInElement(WebDriver driver, By locator) {
        WebDriverWait wait = new WebDriverWait(driver, UtilPage.Implicit_Wait);
        String text = null;
        int attempts = 0;
        while (attempts++ < 5) {
            try {
