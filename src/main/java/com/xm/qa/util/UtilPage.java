...
import com.xm.qa.util.WebDriverUtilities;
...
public class UtilPage {
	...
	//protected Logger log = Logger.getLogger(UtilPage.class);
	...
	public boolean isWebElementDisplayed(WebElement element) {
		try {
			WebDriverWait waiting = new WebDriverWait(driver, Page_Timeout);
    ...
    public WebElement waitUntilEitherOfTheWebElementDisplayed(WebElement element1, WebElement element2) {
		...
		try {
			WebDriverWait waiting2 = new WebDriverWait(driver, Page_Timeout);
	...
	public void handleAlert() {
		WebDriverWait waiting = new WebDriverWait(driver, Implicit_Wait);
		waiting.until(ExpectedConditions.alertIsPresent());
		driver.switchTo().alert().accept();
	}
	
	/**
	 * Compare web elements
	 * 
	 * @param element
	 * @param compareTo
	 * @return
